...
const deleteGreeting = async (req, res) => {
    ...
    try {
        await client.connect();
        const r = await db.collection('two').deleteOne({_id: _id});
        assert.equal(1, r.deletedCount);
        res.status(204).json({ status: 204, _id, message: 'deleted' });
    } catch (err) {
        console.log(err.stack)
        res.status(500).json({ status: 500, data: req.body, message: err.message });
    }
...
const updateGreeting = async (req, res) => {
    ...
    try {
        ...
        const r = await db.collection('two').updateOne(query, newValue);
        assert.equal(1, r.deletedCount);
        assert.equal(1, r.modifiedCount);
        res.status(200).json({ status: 204, _id, message: 'updated' });
    } catch (err) {
        console.log(err.stack)
        res.status(500).json({ status: 500, data: req.body, message: err.message });
    }
...
module.exports = { updateGreeting, createGreeting, getGreeting, getGreetings, deleteGreeting };